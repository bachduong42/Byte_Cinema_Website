...
const ChangeOtp = ({ setModalRef, openLoginModal }) => {
    ...
    useGSAP(() => {
        ...
        tl.to(overlayRef.current, { display: 'flex' })
            // .call(startCountdown)
            .from(overlayRef.current, { duration: 0.3, backgroundColor: 'rgba(0,0,0,0)', ease: 'expo.out' })
    ...
    return (
        <>
            <div id="fp-overlay" ref={overlayRef} className='fixed z-5 inset-0 bg-[rgba(0,0,0,0.5)]  items-center justify-center hidden' onMouseDown={handleOverlayClick}>
                <div id="fp-outer-box" ref={outerBoxRef} className='relative w-[30rem] items-center justify-center self-center'>
                    ...
                    <div id="fp-inner-box" className='relative z-2 bg-white rounded-xl h-full p-[20px]'>
                        <div id="close-div" className='w-full h-max mb-[1rem] flex justify-end' onClick={closeConfirmOtpModal}>
                            ...
                            <div className='w-full p-[20px]'>
                                ...
                                <div className="input mb-[1.5rem] mt-[1rem]">
                                    <div className='relative flex-row'>
                                        <input type="text" name="otp" placeholder="Nhập OTP" required className=' w-8/12 px-[15px] py-[10px] bg-[#f8f6f6] rounded-xl   focus:outline-none focus:border focus:border-[#db9a45] pr-[88px]' onChange={(e) => setOtp(e.target.value)} onBlur={validateOtp} value={otp} maxLength={6} onKeyPress={(e) => {
                                            if (!/[0-9]/.test(e.key)) {
                                                e.preventDefault();
                                            }
                                        }} />
                                        ...
                                        {otp.length > 0 && (
                                            <div className={`absolute top-[50%] transform -translate-y-[3px] right-[30%] -translate-x-[10px]`} onClick={() => setOtp("")}>
                                                <IonIcon icon={closeSharp} className='text-[rgba(0,0,0,0.5)] text-2xl cursor-pointer' />
                                            </div>
                                        )}

                                        <button type='button' className='w-1/4 ml-[0.5rem] text-base p-[10px] mt-[1rem] bg-[#e3e3e3] text-[rgba(0,0,0,0.5)] border-none cursor-pointer rounded-xl transition-all duration-500 ease-in-out' disabled={isResendButtonDisabled} style={{ backgroundColor: isResendButtonDisabled ? '#e3e3e3' : '#db9a45' }} onClick={handleSendOtp}>{isResendButtonDisabled ? `${String(resendTimeout).padStart(2, '0')}s` : 'Gửi lại OTP'}</button>
                                    </div>
